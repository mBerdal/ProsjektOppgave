...
\begin{equation}\label[eq]{local_prob}
  \hat{p}:(\mathbb{R}^{2}, \mathbb{R}^{2}, \mathbb{R}^{+})\rightarrow [0, 1]\quad\hat{p}(\mathbf{x}_{a}, r_{a}, \mathbf{y}) = \begin{cases}
    p(\norm{\mathbf{x}_{a}-\mathbf{y}}) &, \mathbf{y}\in V(\mathbf{x}_{a}, r_{a})\\
    0 &, \mathbf{y}\in\mathbb{R}^{2}\setminus V(\mathbf{x}_{a}, r_{a})
  \end{cases}
\end{equation}
Where $V(\mathbf{x}_{a}, r_{a})$ denotes the \textit{visible set} of agent $i$. Assuming line-of-sight (LoS) communication, meaning an agent
